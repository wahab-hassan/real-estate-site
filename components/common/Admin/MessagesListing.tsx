...
const MessagesListing = () => {
  ...
  const [isRead, setIsRead] = useState<any>(null);
  ...
  const deleteUser = async (userId: any) => {
    const isConfirm = confirm("Are you sure you want to delete this messages?");
    if (isConfirm) {
      await deleteRecord("messages", userId);
      fetchUsers(page, limit);
  ...
  const handleFilter = async () => {
    const filters = [];
    if (isRead) filters.push({ key: "is_read", value: isRead });
  ...
  const getCount = async () => {
    const count: any = await getTotalMessages("messages");
    setTotalUsers(count);
  };
            ...
            <table className="w-full text-sm text-left rtl:text-right h-auto my-2 border ">
              <thead className="text-gray-700 uppercase bg-gray-50  ">
                <tr>
                  ...
                  <th
                    scope="col"
                    className="px-6 py-5 text-[12px] text-wrap w-32 cursor-pointer"
                  >
                    Message{" "}
                  </th>
                  <th
              ...
              <tbody>
                {users?.map((user: any) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3">{user.message}</td>
                    <td className="px-6 py-3">
                      <div className="flex items-center">
                        <Switch
                          checked={user.is_read}
